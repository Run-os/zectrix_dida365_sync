...
| status(0/2)                    | status(0/1)       | status=2 映射为 1（完成），否则为 0（未完成）。                                                                                                                                                                                       |

补充说明：

- 该函数返回 status，不返回 completed；completed 仅在同步层用于比较/状态切换。
...
更具体地说，完成态联动发生在常规双向更新之前：

1. 若 Zectrix 已完成、DIDA 未完成：调用 DIDA complete 接口并更新内存中的 dida_task.status=2。
2. 若 Zectrix 带 DIDA 标记但 DIDA 任务不存在：尝试调用 Zectrix complete 接口，将该 todo 标记完成。

### 5.4 DIDA365 -> Zectrix 更新判定

同一任务（通过 DIDA ID 关联）进入更新分支前，按如下顺序判定：
...
5. 若未关联到 Zectrix todo，则执行 create_todo。

注意：状态切换是独立 API（/todos/{todoId}/complete），不是依赖 update_todo 直接写 completed。

### 5.5 Zectrix -> DIDA365 更新判定

同一任务（通过 description 标记解析出的 dida_id）进入更新分支前，按如下顺序判定：

1. 配置过滤：当 sync_completed=false 且 Zectrix completed=true，直接跳过。
2. 字段指纹比较：若 title/content/dueDate/status 四字段一致，直接跳过。
3. 上次同步保护：若 zectrix.updateDate <= last_sync_completion_time，跳过，避免上一轮写回触发反向覆盖。
4. 时间戳比较：仅当 zectrix.updateDate > dida.modifiedTime 时，执行 update_task。
5. 更新前补齐必要字段：确保 id、projectId、title 存在。
6. 若 dida_id 不存在或未命中 dida_task_map，则执行 create_task，并强制写入 DIDA_PROJECT_ID。
...
每轮 sync 完成后会写入 sync_state.json：
...
程序会在项目根目录维护 sync_state.json，核心字段如下：
